...
There are `n+2` positions numbered from `0` to `n+1`. Initially, position `i` contains an element of weight `wᵢ` for every `1≤ i≤ n`, while positions `0` and `n+1` are empty.

You choose an integer `k`. Then every element moves exactly once, simultaneously:

 - If `wᵢ < k`, the element at position `i` moves to position `i-1`;
- If `wᵢ > k`, the element at position `i` moves to position `i+1`;
- If `wᵢ=k`, the entire movement process fails immediately.
...
The second line of each test case contains `n` integers `w₁,w₂,…,wₙ` (`1≤ wᵢ≤ 10⁹`).
