...
In the second test case, Ja has only one possible operation: [7,3]→ [3,10].The sequence is then sorted, so the answer is10.

In the third test case, Ja can perform the following operations: [3,2,1]→ [2,5,1]→ [2,1,6]→ [1,3,6].The largest pile contains6ducks. If Ja first chooses the last two piles instead, the final largest pile would contain7ducks. Therefore the answer is6.

In the fourth test case, Ja cannot choose the first two piles at the beginning, because 2 is not greater than 2. One possible process is [2,2,1,3,3]→ [2,1,3,3,3]→ [1,3,3,3,3].Thus the answer is3.

In the fifth test case, one optimal process is [3,1,4,2]→ [1,4,4,2]→ [1,4,2,6]→ [1,2,6,6].Therefore the answer is6.
