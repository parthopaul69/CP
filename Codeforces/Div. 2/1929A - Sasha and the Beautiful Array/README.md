...
Sasha decided to give his girlfriend an array `a₁, a₂, …, aₙ`. He found out that his girlfriend evaluates the *beauty* of the array as the sum of the values `(aᵢ - aᵢ ₋ ₁)` for all integers `i` from `2` to `n`.
...
The second line of each test case contains `n` integers `a₁, a₂, …, aₙ` (`1 ≤ aᵢ ≤ 10⁹`) — the elements of the array `a`.
...
In the first test case, the elements of the array `a` can be rearranged to make `a = [1, 2, 3]`. Then its beauty will be equal to `(a₂ - a₁) + (a₃ - a₂) = (2 - 1) + (3 - 2) = 2`.
