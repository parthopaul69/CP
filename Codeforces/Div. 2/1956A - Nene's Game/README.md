...
Nene has not yet decided how many people would join the game initially. Nene gave you `q` integers `n₁, n₂, …, nq` and you should answer the following question for each `1 ≤ i ≤ q` **independently**:
...
The first line case contains two integers `k` and `q` (`1 ≤ k, q ≤ 100`) — the length of the sequence `a` and the number of values `nᵢ` you should solve this problem for.

The second line contains `k` integers `a₁,a₂,…,aₖ` (`1≤ a₁ < a₂ < … < aₖ≤ 100`) — the sequence `a`.

The third line contains `q` integers `n₁,n₂,…,nq` (`1≤ nᵢ ≤ 100`).
