...
You are given a single integer `n.` Construct an array of `n` distinct positive integers `a₁, …, aₙ` such that for all `i (1 ≤ i ≤ n)`, `a₁ + a₂ + a₃ + … + aₙ` is divisible by `aᵢ,` or determine that no such array exists.
...
Otherwise, output `n` integers `a₁, …, aₙ (1 ≤ aᵢ ≤ 10¹⁷)` — an array satisfying the conditions.
...
In the second test case, the output is `a = [1,9,2,6].` We have `a₁ + a₂ + a₃ + a₄ = 18,` which is divisible by all `aᵢ.`
