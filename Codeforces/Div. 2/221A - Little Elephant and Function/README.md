...
This time he enjoys the sorting function. Let `a` is a permutation of an integers from 1 to `n`, inclusive, and `aᵢ` denotes the `i`-th element of the permutation. The Little Elephant's recursive function `f(x)`, that sorts the first `x` permutation's elements, works as follows:

 - If `x = 1`, exit the function.
- Otherwise, call `f(x - 1)`, and then make `swap(aₓ ₋ ₁, aₓ)` (swap the `x`-th and `(x - 1)`-th elements of `a`).
