...
The first line of each test case contains two integers `n` and `m` (`1 ≤ n, m ≤ 2 · 10⁵`) — the lengths of the arrays `a` and `b`.

The second line contains `n` integers `a₁, a₂, …, aₙ` (`1 ≤ aᵢ ≤ 10⁹`).

The third line contains `m` integers `b₁, b₂, …, bₘ` (`1 ≤ bᵢ ≤ 10⁹`).
...
It is guaranteed that the sum of `m` over all test cases does not exceed `2 · 10⁵`.
