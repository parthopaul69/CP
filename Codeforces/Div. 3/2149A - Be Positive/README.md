...
Given an array `a` of `n` elements, where each element is equal to `-1`, `0`, or `1`. In one operation, you can choose an index `i` and increase `aᵢ` by `1` (that is, assign `aᵢ := aᵢ + 1`). Operations can be performed any number of times, choosing any indices.

The goal is to make the product of all elements in the array strictly positive with the minimum number of operations, that is, `a₁ · a₂ · a₃ · … · aₙ > 0`. Find the minimum number of operations.
...
The first line contains one integer `t` (`1 ≤ t ≤ 10⁴`) — the number of test cases. The description of the test cases follows.

The first line of each test case contains one integer `n` (`1 ≤ n ≤ 8`) — the length of the array `a`.

The second line contains `n` integers `a₁, a₂, …, aₙ`, where `-1 ≤ aᵢ ≤ 1` — the elements of the array `a`.
