...
In one operation, Akito chooses two integers `i` and `x` such that `1 ≤ i ≤ n` and `-p ≤ x ≤ p`, and performs the assignment `aᵢ = x`.
...
In the seventh example, only one operation `a₃ = -7` is needed.
