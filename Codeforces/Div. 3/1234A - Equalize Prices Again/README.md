...
You are both a shop keeper and a shop assistant at a small nearby shop. You have `n` goods, the `i`-th good costs `aᵢ` coins.
...
The first line of the query contains one integer `n` (`1 ≤ n ≤ 100)` — the number of goods. The second line of the query contains `n` integers `a₁, a₂, …, aₙ` (`1 ≤ aᵢ ≤ 10⁷`), where `aᵢ` is the price of the `i`-th good.
