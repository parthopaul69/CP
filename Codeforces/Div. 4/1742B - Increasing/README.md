...
You are given an array `a` of `n` positive integers. Determine if, by rearranging the elements, you can make the array strictly increasing. In other words, determine if it is possible to rearrange the elements such that `a₁ < a₂ < … < aₙ` holds.
...
The second line of each test case contains `n` integers `aᵢ` (`1 ≤ aᵢ ≤ 10⁹`) — the elements of the array.
