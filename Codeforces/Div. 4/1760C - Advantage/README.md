...
Every participant wonders how much of an advantage they have over the other best participant. In other words, each participant `i` wants to know the difference between `sᵢ` and `sⱼ`, where `j` is the strongest participant in the competition, not counting `i` (a difference can be negative).

So, they ask you for your help! For each `i` (`1 ≤ i ≤ n`) output the difference between `sᵢ` and the maximum strength of any participant other than participant `i`.
...
The following line contains `n` space-separated positive integers `s₁`, `s₂`, ..., `sₙ` (`1 ≤ sᵢ ≤ 10⁹`) — the strengths of the participants.

It is guaranteed that the sum of `n` over all test cases does not exceed `2·10⁵`.

## Output

For each test case, output `n` space-separated integers. For each `i` (`1 ≤ i ≤ n`) output the difference between `sᵢ` and the maximum strength of any other participant.
