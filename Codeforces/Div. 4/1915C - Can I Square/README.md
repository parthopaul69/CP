...
Calin has `n` buckets, the `i`-th of which contains `aᵢ` wooden squares of side length `1`.
...
The first line of each test case contains a single integer `n` (`1 ≤ n ≤ 2 · 10⁵`) — the number of buckets.

The second line of each test case contains `n` integers `a₁, …, aₙ` (`1 ≤ aᵢ ≤ 10⁹`) — the number of squares in each bucket.
