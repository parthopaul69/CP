...
Given an array `a=[a₁,a₂,…,aₙ]` of `n` positive integers, you can do operations of two types on it:

 - Add `1` to **every** element with an **odd** index. In other words change the array as follows: `a₁ := a₁ +1, a₃ := a₃ + 1, a₅ := a₅+1, …`.
- Add `1` to **every** element with an **even** index. In other words change the array as follows: `a₂ := a₂ +1, a₄ := a₄ + 1, a₆ := a₆+1, …`.
...
Note that after the performed operations the elements in the array can become greater than `10³`.
